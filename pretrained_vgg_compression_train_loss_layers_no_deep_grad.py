...
class myModel(nn.Module):
    def __init__(self, insertion_layer):
        ...
        vgg_model = VGGModel()	
        vgg_model.load_state_dict(torch.load('/home/leconte/Documents/information_quantization/vggmodel.pt'))
        self.final_list = []
        self.final_list_classif = []
        self.is_compression = insertion_layer - 1

        for i, layer in enumerate(list(vgg_model.features.children())):
            if i != insertion_layer - 1 :
...
def insertion_and_train(ins):
    ...
    model = myModel(ins).to(device)
    quantized_grad_w = compression_model(model.final_list[model.is_compression].out_channels*model.final_list[model.is_compression].in_channels, args.num_embeddings, args.embedding_dim , commitment_cost).to(device)
    quantized_grad_b = compression_model(model.final_list[model.is_compression].out_channels, args.num_embeddings, args.embedding_dim , commitment_cost).to(device)
    # optimizer = optim.SGD(model.parameters(), args.learning_rate, momentum = 0.9, weight_decay =  5e-4)
    optimizer = optim.Adam([quantized_grad_w.parameters(), quantized_grad_b.parameters()], args.learning_rate)
    ...
    epochs_train_res_recon_error_b = []
    epochs_train_res_perplexity_b = []
    epochs_train_res_vq_loss_b = []
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        quantized_grad_w.train()
        quantized_grad_b.train()
        ...
        ### the codebook is updated only 1/20 times
            if i%1==0:
                quantized_grad_w._vq_vae.update_codebook = True
                quantized_grad_b._vq_vae.update_codebook = True
            else:
                quantized_grad_w._vq_vae.update_codebook = False
                quantized_grad_b._vq_vae.update_codebook = False
            ...
            for i, layer in enumerate(list(model.features.children())):
                for param in layer.parameters():
                    if param.requires_grad == True:
                        grad_before = param.grad
                        if len(grad_before.size()) == 1:
                            grad_before_b = grad_before.view(grad_before.size(0), 1, 1, 1)
                            outc, inc, k0, k1 = grad_before_b.size(0), grad_before_b.size(1), grad_before_b.size(2), grad_before_b.size(3)
                            grad_before_b = grad_before_b.view(1, outc*inc, k0, k1)
                            vq_loss_b, grad_recon_b, perplexity_b = quantized_grad_b(grad_before_b)  # or whatever other operation
                            grad_recon_b = grad_recon_b.view(outc, inc, k0, k1)
                            param.grad = grad_recon_b
                        else:
                            outc, inc, k0, k1 = grad_before.size(0), grad_before.size(1), grad_before.size(2), grad_before.size(3)
                            grad_before_w = grad_before.view(1, outc*inc, k0, k1)
                            vq_loss_w, grad_recon_w, perplexity_w = quantized_grad_w(grad_before_w)  # or whatever other operation
                            grad_recon_w = grad_recon_w.view(outc, inc, k0, k1)
                            param.grad = grad_recon_w                           
                        
            grad_variance_w = torch.var(grad_before_w)

            recon_error_w = F.mse_loss(grad_recon_w, grad_before_w) / grad_variance_w
            loss_w = recon_error_w + vq_loss_w

            loss_w.backward()
            
            grad_variance_b = torch.var(grad_before_b)

            recon_error_b = F.mse_loss(grad_recon_b, grad_before_b) / grad_variance_b
            loss_b = recon_error_b + vq_loss_b

            loss_b.backward()
            
            optimizer.step()
            
            train_res_recon_error.append(recon_error_w.item())
            train_res_perplexity.append(perplexity_w.item())
            train_res_vq_loss.append(vq_loss_w.item())
            train_res_recon_error_b.append(recon_error_b.item())
            train_res_perplexity_b.append(perplexity_b.item())
            train_res_vq_loss_b.append(vq_loss_b.item()) 
        ...
        print('recon_error: %.3f' % np.mean(train_res_recon_error_b))
        print('perplexity: %.3f' % np.mean(train_res_perplexity_b))
        print('vq_loss: %.3f' % np.mean(train_res_vq_loss_b))
        print()
        ...
        epochs_train_res_recon_error_b.append(np.mean(train_res_recon_error_b))
        epochs_train_res_perplexity_b.append(np.mean(train_res_perplexity_b))
        epochs_train_res_vq_loss_b.append(np.mean(train_res_vq_loss_b))
    ...
    return epochs_train_res_recon_error_b, epochs_train_res_perplexity_b, epochs_train_res_vq_loss_b, epochs_train_res_recon_error, epochs_train_res_perplexity, epochs_train_res_classif_loss, epochs_train_res_vq_loss, epochs_Acc1, epochs_Acc5, train_epochs_Acc1, train_epochs_Acc5
...
def main():
    ...
    for ins in [1, 3, 6, 8, 11, 13, 15, 18, 20, 22, 25, 27, 29]:
        ...
        print(ins)
        recon_error_b, perplexity_b, vq_loss_b, recon_error, perplexity, classif_loss, vq_loss, Acc1, Acc5, train_Acc1, train_Acc5 = insertion_and_train(ins)
     
        epochs_train_res_recon_error_b.append(recon_error_b)
        epochs_train_res_perplexity_b.append(perplexity_b)
        epochs_train_res_vq_loss_b.append(vq_loss_b) 
