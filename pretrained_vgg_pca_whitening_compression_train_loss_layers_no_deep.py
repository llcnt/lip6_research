    ...
    def forward(self,x, mu, u, s):
        for i, layer in enumerate(self.features):
            if i == self.is_compression:
                ...
                x = x - mu
                x = x.view(b, c, -1)
                x = torch.matmul(torch.matmul(torch.diag_embed(1/torch.sqrt(s+1e-5)), u.transpose(1, 2)), x.permute(2, 1, 0).contiguous())
                ...
                x = x.view(b, c, -1)
                x = torch.matmul(torch.matmul(u, torch.diag_embed(torch.sqrt(s+1e-5))), x.permute(2, 1, 0).contiguous())
...
def insertion_and_train(ins):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, target) in enumerate(training_loader): #50000/256 ~ 200 steps 
            ...
            vq_loss, output, perplexity, data_before, data_recon = model(images, mu.to(device), u.to(device), s.to(device))
